...
toc-expand: true
# Content overwritten from an earlier version - 2025-05-29 11:27
---
...
#### Why
### Features of Flag Utility Module
...
#### 1. .env.example File Update

The .env.example file now includes a FEATURE_FLAGS JSON string. This configuration is intended for offline environments where the deployment cannot be integrated with LaunchDarkly. This enables the offline evaluation of feature flags by loading a pre-defined JSON of flags.

#### 2. Update to feature_flags.py

The feature_flags.py file has been updated with several new functions. A new function, get_offline_feature_flag, retrieves flag values from the offline configuration, returning a default if the flag is not present. Additionally, get_feature_flag_with_context first checks the offline configuration before attempting a LaunchDarkly evaluation, enhancing reliability in offline mode.

#### 3. Context Functions Improvement

Two context functions have been improved. The existing context_from_user function now has an enhanced docstring, and a new function, context_from_user_and_organization, is added to create a multi-context using both user and organization information. This allows for more granular feature flag evaluations based on multiple context sources.

#### 4. Backend Tracking Route Update

The tracking route in the backend now leverages the new get_feature_flag_with_context function instead of directly accessing the ldclient. This decouples the feature flag evaluation from the LaunchDarkly client and allows for more flexible offline support.

#### 5. Unit Test Enhancements

Comprehensive unit tests have been added in tests/unit/test_feature_flags.py. These tests cover various scenarios, including context creation, offline and online flag evaluation, and handling of scenarios where user or organization data may be missing.
...
#### Why
### Workflow async task on model field change is not executing

#### Issue Description
I am facing an issue where the workflow async task on a model field change is not executing as expected.

#### Steps to Reproduce
1. Update the value of the specific field in the model.
2. Check if the workflow async task is triggered.

#### Expected Behavior
The workflow async task should be executed when the model field is changed.

#### Actual Behavior
The workflow async task does not seem to be triggered upon changing the model field.

##### Additional Information
- Framework: Django 3.2
- Workflow Configuration: Celery with Redis backend
...
In the 'celery_config.py' file, the worker concurrency has been increased from 4 to 6. This adjustment aims to enhance parallel task processing to alleviate performance bottlenecks during high application workloads.

##### 2. Logging Enhancement in Workflow Tasks
A new logging statement is now present in the 'workflows.py' module. This log captures the workflow discovery process triggered by a field change, detailing the field key and the model's associated organization. This addition facilitates debugging and monitoring of workflow queries.
...
#### Changes in Workflow Retrieval Methods
...
#### What
### Issue: Unauthorized Escalation of User Permissions

#### Description
We have identified a security vulnerability where regular users are able to elevate their permissions by granting themselves admin access.
...
The variable `user` has been renamed to `to_update_user` for clarity in representing the user being updated. This change ensures consistency in organization membership checks and role assignment creation.

##### Event Publishing Adjustment
The event publishing call now correctly references the updated user variable, guaranteeing that events are generated with the correct user information.
...
- Removal of the legacy AUTH_AUTHORITY variable during environment setup to align with the new configuration.
...
#### Changes to API Endpoint for 'Contents' Field in UI Inventory Models
...
In the backend API module, the unused import of flask_migrate has been removed, cleaning up the dependencies of the project.
...
#### Modifications in docker-compose.yaml
...
#### Improvements in docker-entrypoint.sh
...
#### Breaking Changes
### Considerations for Role-Based Access Control Updates
...
#### Changes in Authentication and Authorization Logic

##### New Function: get_token_auth_cookie
...
The function get_token_auth_header_or_cookie combines both header and cookie approaches to retrieve the token, ensuring flexibility in token extraction. This function is then used in place of the previous get_token_auth_header method in the authentication decorators, thus allowing fallback from header to cookie.

##### Authentication Enforcement in UI Assets Proxy
In the asset serving endpoint within the UI Assets Proxy, an authentication decorator (@requires_auth()) is added to enforce that the API is accessed by authenticated users.

##### Authorization Check in Asset Serving
Additionally, the endpoint now performs an authorization check by validating that if the asset key contains an organization identifier, it must match the current user's organization. If there is a mismatch, an appropriate unauthorized error is raised.
...
#### Changes to Deployment Workflow Configurations

##### AWS Secondary Environment

This pull request updates the deployment workflow configurations for the AWS secondary environment. Instead of copying the old template file (values.yaml), the workflow now copies the new template file (values-new.yaml) into the manifests directory. The sed commands for performing template substitutions remain unchanged. This change ensures that deployments in the AWS secondary environment utilize the updated configuration template for newer deployment requirements.

##### AWS Production Environment

This pull request updates the deployment workflow configurations for the AWS production environment. Instead of copying the old template file (values.yaml), the workflow now copies the new template file (values-new.yaml) into the manifests directory. The sed commands for performing template substitutions remain unchanged. This change ensures that deployments in the AWS production environment utilize the updated configuration template for newer deployment requirements.

##### AWS Staging Environment

This pull request updates the deployment workflow configurations for the AWS staging environment. Instead of copying the old template file (values.yaml), the workflow now copies the new template file (values-new.yaml) into the manifests directory. The sed commands for performing template substitutions remain unchanged. This change ensures that deployments in the AWS staging environment utilize the updated configuration template for newer deployment requirements.
...
#### External Release Notes
### When creating a new workflow, a new Trigger type is available for "On Model Field Change"
...
### Changes in PR for Approval Vote Activity

#### Overview
This PR introduces the display of the reviewer's avatar, name, and notes within the approval vote activity, integrated inline with the existing text. The rendering of the voter leverages the approval voter columns list component for consistency.

*Note: This implementation is exploratory and subject to design feedback. Any suggestions from the design team are welcome.*

#### Before (Dashboard Recent Activity)
![activity-voted-before](https://github.com/user-attachments/assets/1d223bca-e325-41a5-b711-bb6279734ef5)

#### After (Dashboard Recent Activity)
...
#### From Inventory Model Activity Tab
...
#### Why
### Explanation of Customer Comment Viewing Option

#### Reasons for Change
Customers have expressed a desire to be able to view comments that have been made.
...
##### Note
- The roles details page is reused for the stakeholder type detail page with slight differences. Also, the API roles delete endpoint is reused for stakeholder type deletion requests.

#### External Release Notes
### Added the ability to delete Roles and Stakeholder Types

#### Screenshots/Videos (Frontend Only)
### [Link to User Attachments Assets](https://github.com/user-attachments/assets/fa8dd882-6067-4a76-9d87-55013d912464)
...
#### Standardized Deletion Confirmation Workflow
...
##### Overview
This PR focuses on improving the error handling logic within the BootstrapForm component. The primary enhancement involves prioritizing the display of detailed error messages obtained from the API response. By extracting error messages from error.response?.data?.message, the form now provides more specific and informative feedback to users in cases of organization creation failures.

##### Details
The updated code introduces a local variable (errorMessage) to store the chosen error message consistently. This variable is utilized for both inline error displays and toast notifications, ensuring uniformity in error messaging. These changes aim to enhance troubleshooting processes and improve user guidance by utilizing more descriptive error messages supplied by the backend.
...
#### will no longer have default text blocks for a given section

##### Instead an empty state will be present prompting the user to "Add New Block"
...
#### This is a standard security enhancement requiring only the addition of the DOMPurify package.
...
#### External Release Notes
### Added support for authentication via cookies when accessing assets
This improves the display of images and other media in documentation, reports, and other sections of the application.
...
##### Preparation for Validation Experience Notebooks

[**Coming Soon**]

#### # Other stuff
### Feedback on Website Updates
