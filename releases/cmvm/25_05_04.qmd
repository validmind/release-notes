...
toc-expand: true
# Content overwritten from an earlier version - 2025-06-01 17:07
---
...
#### Why
### Explanation of Customer Request
...
#### 1. Annotation Model Updates

In the Annotation model (annotation.py), a new static method, get_user_comments_by_model_id, is added to query and join annotation data with related comments and the user who created them.

#### 2. Report Serialization Process Changes

The report serialization process is updated across several modules. Changes in model_inventory.py and reports.py now support an additional flag ('include_comments') for embedding user comments into reports, affecting both the document generation and the content structure.

#### 3. UI Reporting Routes Enhancement

The UI reporting routes (ui_reporting.py) now parse an additional parameter for comments inclusion. A helper method, parse_generate_report_params, consolidates all report parameters including the new include_comments flag.

#### 4. Report Render Helpers Addition

In the report render helpers, a new function get_comment_text_for_doc is introduced. This function processes the retrieved comments, stripping HTML formatting and organizing them in a styled text block that is then converted to a document fragment.

#### 5. ExportAnnotation Dataclass

A new dataclass ExportAnnotation in util_models.py is added to facilitate clean export of comment details including user name, text content, created date, and context.

#### 6. Unit Testing

Several unit tests were added in test_reports.py to verify the functionality: testing serialization with comments, ensuring that comments appear as expected in document fragments, and validating that HTML is properly cleaned before being embedded in the final report.
...
#### What
### Changes in Feature Flags Codebase

#### Offline Feature Flag Support
...
#### Why
### Features of the FlagHandler Module
...
#### Section 1: Offline Flag Support

##### Subsection 1.1: .env.example Configuration
The .env.example file now includes a FEATURE_FLAGS JSON string for offline environments.

##### Subsection 1.2: Updated feature_flags.py
The feature_flags.py file has new functions for offline flag retrieval and improved context handling.

#### Section 2: Improved Context Functions

##### Subsection 2.1: Enhancements to Existing Context Function
The context_from_user function now has an enhanced docstring.

##### Subsection 2.2: New Multi-Context Function
A new function, context_from_user_and_organization, is added to create a multi-context using user and organization information.

#### Section 3: Backend Tracking Route Update

##### Subsection 3.1: Leveraging New Function
The tracking route in the backend now uses the get_feature_flag_with_context function for improved flexibility.

#### Section 4: Unit Testing

##### Subsection 4.1: Comprehensive Unit Tests
Comprehensive unit tests have been added in tests/unit/test_feature_flags.py to cover various scenarios.
...
In the Celery configuration, the worker concurrency has been increased from 4 to 6. This change aims to enable more tasks to run concurrently, thereby addressing performance bottlenecks during high application workloads. The modification can be found in the 'celery_config.py' file.

##### Logging Enhancement in Workflow Tasks Module
A new logging statement has been incorporated into the workflow tasks module ('workflows.py') to log the process of identifying available workflows based on a field change. This log entry includes details such as the field key and the organization linked to the model. The addition is expected to facilitate debugging and monitoring the execution flow during workflow queries.
...
#### What
### Description
#### Change in Validation for `PUT /api/v1/statuses-workflows/{id}/status` Endpoint

This change adds additional validation to the `PUT /api/v1/statuses-workflows/{id}/status` endpoint. The purpose of this validation is to prevent users from modifying the workflow status of organizations to which they do not belong.
...
#### Enhancements to User Role Management Endpoints

##### Authorization Mechanism Update
In the POST endpoint, the authorization decorator has been refined to use `resource="Role"` and `action="create"` for improved control. Similarly, the DELETE endpoint now utilizes `resource="Role"` with `action="delete"` to align with permission requirements.

##### Variable Naming Clarity
The variable `user` has been renamed to `to_update_user` for better clarity in representing the user being updated. This change ensures consistency in organization membership checks and role assignment creation.

##### Event Publishing Adjustment
The event publishing call now correctly references the updated user variable, guaranteeing that events are generated with the accurate user information.
...
#### What
### The api route `/api/v1/workflows/targets/{cuid}/executions` calls `Workflow.get_workflow` and `Execution.get_by_target`
#### The first call passes the user and only selects workflows within that user's organization, but the second selects executions by target (model) from all organizations
...
#### In src/backend/db/workflow.py
- The get_workflow method signature is updated to accept an organization and a cuid. It now checks if the organization is provided, raising a ValueError with a descriptive message if it is not.
...
#### In the REST API endpoints in src/backend/routes/ui_workflows.py
- All calls to Workflow.get_workflow have been updated to pass g.current_user_org instead of g.current_user. This aligns the data retrieval with the new workflow model requirements.

#### The corresponding unit tests in tests/unit/test_workflows.py
- Have been modified to use the organization parameter when fetching workflows and testing execution queries. This ensures consistency in how workflows are retrieved throughout the project.
...
#### 1. Volume Mounts Removal
The volume mounts for the Keycloak configuration (realm-export.json, themes, and entrypoint.sh) in the docker-compose file have been removed. Instead, these files are now copied into the Docker image via updates in the Dockerfile. This change aims to streamline deployment by baking in the required assets during image build time.

#### 2. Environment Variable Modification
An environment variable was modified from `KC_DB_DATABASE` to `KC_DB_DtATABASE`. This appears to be either an intentional change or a typo. It should be verified whether the new variable name is correct and consistent with the expected configuration in downstream components.
...
- Install additional utilities (jq, postgresql-client-15, redis-tools) that support enhanced operations and debugging.

#### Modifications in the docker-compose.yaml

- Add a new port mapping (9000) for Keycloak.
- Set a new environment variable (KC_HEALTH_ENABLED) to enable health checking.

#### Improvements in the docker-entrypoint.sh

- Implement readiness checks for Postgres, Redis, and, conditionally, Keycloak to ensure that dependent services are available before the application starts. This includes waiting loops with clear logging messages for each service.

#### Update to the Keycloak Dockerfile

- Expose the new port (9000) alongside the existing one.
...
#### Enhancements:
- Adding authentication requirement to the endpoint that was previously unprotected
- Implementing organization-level access control to ensure users can only access assets from their own organization
- Adding support for auth token retrieval from cookies as a fallback when the Authorization header is not present
- Adding validation logic that checks if the requested asset belongs to the user's organization
...
##### Authentication and Organization-Level Access Controls
...
#### Areas Needing Special Review
### Authentication flow with token fallback from cookies
#### Organization-level access control implementation
...
#### In the backend API configuration (src/backend/api.py)

The assignment of the SQLALCHEMY_MAX_OVERFLOW setting has been reformatted to improve code readability. Although the functionality remains the same, the code style is now consistent with other multiline assignments.

#### In the template retrieval logic (src/backend/routes/ui_templates.py)

The main functional enhancement is in the template retrieval logic. Previously, the code queried for a template solely based on a unique identifier. With the changes, the query now also filters based on the current user's organization (using g.current_user_org.id), ensuring that users can only access templates that belong to their own organization. This adjustment enhances both data integrity and security.

#### Minor import reordering and cleanup in src/backend/routes/ui_assets_proxy.py

Minor import reordering and cleanup have also been performed in src/backend/routes/ui_assets_proxy.py, which improves the overall code organization and clarity.
...
#### Migration Script for `pg_stat_statements` Extension

##### Overview
This PR introduces a migration script aimed at managing the `pg_stat_statements` extension in a PostgreSQL database. The migration checks for the existence of the extension and creates it if it is absent.

##### Upgrade Function
The upgrade function connects to the database and executes a query to determine if the extension is already installed. If not, it proceeds to create the extension using a conditional SQL command. It then retrieves and prints the current version of the extension for informational purposes.

##### Downgrade Function
The downgrade function is currently a placeholder and does not implement any rollback logic.
...
#### Why
### Explanation of Customer Comment Viewing Feature
...
- Removal of the legacy AUTH_AUTHORITY variable and introduction of the KEYCLOAK_SERVER_URL variable to explicitly define the Keycloak API endpoint.

#### Updates in GitHub Actions Workflow

- Removal of the AUTH_AUTHORITY environment variable and reliance on the updated key configuration variables.
...
#### Changes to Template Serialization Process
...
#### What
### Description
#### Change in Admin Permissions for Modifying Staff Roles
Before the change, admins had the ability to modify staff roles using the `PATCH /api/v1/roles/<protected_role_id>/permissions` endpoint. After the change, admins are restricted from making modifications to staff roles through this endpoint.
...
#### 1. Consistent naming: 
The variable previously named `orgRole` has been renamed to `org_role` throughout the GET and PATCH endpoints to follow snake_case conventions and improve code readability.

#### 2. Enhanced role protection: 
In the PATCH endpoint, an additional check has been introduced to prevent modifications to roles marked as `staff`, similar to the existing constraint for `admin` roles. This ensures that both admin and staff roles remain immutable via this endpoint.

#### 3. Minor refactoring: 
The diff shows a systematic renaming and minor logic adjustments to maintain consistency across related queries and handling of permissions. Overall, these changes should improve maintainability and enforce proper role-based restrictions within the application.
...
#### Modifications to Query Filtering
...
#### What
### Update to Model Membership and Content Type Authorization Logic
...
#### Why
### Previously, users with multiple roles on a model (e.g., both "developer" and "validator") would only have permissions associated with the first role found
#### This could lead to insufficient access and authorization errors for legitimate users
#### The change ensures that all assigned roles are respected, improving correctness and flexibility of the authorization system
...
#### Breaking Changes
### Considerations for Role-Based Access Control Changes
...
#### Changes to Authorization Flow and Role-Based Permissions

##### Replacing Single-Role Lookup with Multi-Role Approach
...
##### Updating Content Type Resolution
...
This PR aims to improve the expressiveness and flexibility of the role-based permission system, which is a key part of the security and integrity of model access and modification operations within the application.
...
#### What
### <!-- Before: Heading level 1 --> Describe what the change is doing, including behavior before and after the change

#### <!-- Before: Heading level 2 --> Section heading

##### <!-- Before: Heading level 3 --> Subsection heading
...
#### UI Assets Proxy Endpoint

##### Simplification of Authorization Checks
...
#### In the EventRowLayout component
Collapsible content has been implemented to display additional details (such as vote notes) when available. An IconButton toggles a Collapse element, and the display icon switches between a downward and upward chevron accordingly.

#### New components related to workflow approvals
...
#### Some redundant code and inline commented sections
Have been cleaned up and replaced with reusable components, improving overall maintainability and readability.
...
#### A new Trigger type is available for "On Model Field Change"
##### This allows selecting an inventory model field to monitor for changes before starting workflow execution

[Link to GitHub Repository](https://github.com/user-attachments/assets/4f0fa1d1-0484-401e-a012-923d4cc6503e)
...
The project version has been bumped (from 1.33.3 to 1.33.4) and new dependencies related to drag-and-drop functionality (such as @minoru/react-dnd-treeview and react-dnd) have been added. These additions enable advanced template manipulation capabilities.

#### API Enhancements

The old SwapTemplate function has been removed and replaced by a new function, CreateAndSwapTemplate, which consolidates the creation and swapping of document templates into a single API call. This improves clarity and reusability in handling template updates.

#### UI Modifications and Component Updates

Several UI components (e.g., AddFindingModal, AttributesRailDocumentOverview, AttributesRailMonitoringOverview, and AttributesRailValidationReportOverview) have been updated to refine user interactions. Notable changes include:
...
A comprehensive new component (SwapTemplateModal) has been introduced. This modal provides a drag-and-drop tree view allowing users to compare the active working template with a selected template version. Users can mark sections and blocks with statuses such as ADD, KEEP, IGNORE, or DELETE, and the UI guides them to resolve any invalid configurations before applying changes.

#### Template Model Adjustments
...
#### Why
### Explanation of Exporting Comments
#### Adding Checkbox in UI
##### Backend Inclusion of Comments in Validation Report
...
#### Screenshots/Videos (Frontend Only)
### [Link to User Attachments Assets](https://github.com/user-attachments/assets/3ffbdfb1-e330-4d8f-be3a-5eb9cb962559)
...
Multiple components across the application have been updated to remove hardcoded color properties from spinners. They now rely on theme defaults (defined in the spinner theme file), ensuring consistent styling.
...
#### Workflow Deletion Modal Improvements

The delete modal for workflows has been improved to handle dependency errors gracefully. If a workflow cannot be deleted due to existing dependencies, a warning alert is shown, along with a list of dependent workflows (with links to their details), and the delete button becomes disabled.

#### New Run Workflow Feature

A new run workflow node, along with its associated panel, has been implemented. This node allows users to trigger another workflow and is integrated into the workflow canvas with proper UI components. It supports configuration via an inline dropdown that paginates the workflow options using the new useWorkflows hook.

#### UI Consistency & Debug Enhancements

Several node components have received minor UI tweaks (e.g., border styling changes and updated rounding) and added inline inspection popovers to aid debugging. Additional components like NodeBox now display contextual data to assist builders in troubleshooting workflows.
...
Note that the roles details page is reused for the stakeholder type detail page with slight differences. Also, the API roles delete endpoint is reused for stakeholder type deletion requests.

#### External Release Notes
### Added the ability to delete Roles and Stakeholder Types

#### Screenshots/Videos (Frontend Only)
### [https://github.com/user-attachments/assets/fa8dd882-6067-4a76-9d87-55013d912464](https://github.com/user-attachments/assets/fa8dd882-6067-4a76-9d87-55013d912464)
...
These changes improve the consistency of deletion flows throughout the application, ensuring that users are well-informed and deliberate when performing potentially destructive actions.
...
#### will no longer have default text blocks for a given section. Instead an empty state will be present prompting the user to "Add New Block"
...
#### What
### Before this PR
#### Any errors raised by the backend are only presented by their error code

### After this PR
#### Helpful error messages from the backend can be presented to the user
##### If no message is presented then the old default message is still presented
...
#### Enhancements to Error Handling Logic in BootstrapForm Component
...
##### Improved Clarity in Error Feedback
...
#### What
### Added DOMPurify library to sanitize HTML content before using dangerouslySetInnerHTML in React components, preventing Cross-Site Scripting (XSS) attacks
#### Before this change, unsanitized HTML was being directly rendered, creating potential security vulnerabilities
#### After the change, all HTML is sanitized through a centralized utility function before rendering
...
#### Additional Notes
### This PR implements DOMPurify version 3.2.5
#### The sanitizeHtml utility function has been configured to allow the "target" attribute on links while sanitizing all other potentially dangerous HTML content.
...
#### Screenshots/Videos (Frontend Only)
### Before/After Screenshots Showing the Improvement in Asset Loading
...
- [Makefile Line 173](https://github.com/validmind/documentation/blob/29c1eb50315ad56b98180836eaf65c47fcdaa487/site/Makefile#L173)

#### Also updated the backup `make execute` command:
- [Makefile Line 125](https://github.com/validmind/documentation/blob/29c1eb50315ad56b98180836eaf65c47fcdaa487/site/Makefile#L125)
...
#### Sidenav
| Documentation guides listing | Validation guides listing | Monitoring guides listing |
| -- | -- | -- |
| <img width="947" alt="Screenshot 2025-05-01 at 5 50 09 PM" src="https://github.com/user-attachments/assets/b1254fb6-acc2-459d-bf8f-afe755509ac3" /> | <img width="1183" alt="Screenshot 2025-05-01 at 5 50 16 PM" src="https://github.com/user-attachments/assets/1f1ab27c-be6c-4100-9d48-c957515d2bfb" /> | <img width="919" alt="Screenshot 2025-05-01 at 5 50 23 PM" src="https://github.com/user-attachments/assets/2595196b-6e2c-4a2d-a202-66ce1f38a909" /> |
...
#### Update Developer Fundamentals Course
...
#### # Other stuff
### Feedback on Recent Website Updates
